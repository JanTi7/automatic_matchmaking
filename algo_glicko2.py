...
def register_game_result(
    game_res_id: str,
    rating_a_1: RatingSnapshot,
    rating_a_2: RatingSnapshot,
    rating_b_1: RatingSnapshot,
    rating_b_2: RatingSnapshot,
    points_a,
    points_b,
    match_importance=1.5,
    use_mif=True,  # TODO put in config
):
    player_a_1 = Glicko2Player(
        rating=rating_a_1.rating,
        rd=rating_a_1.rd_time_adjusted(),
        pid=rating_a_1.player_id,
    )
    player_a_2 = Glicko2Player(
        rating=rating_a_2.rating,
        rd=rating_a_2.rd_time_adjusted(),
        pid=rating_a_2.player_id,
    )
    player_b_1 = Glicko2Player(
        rating=rating_b_1.rating,
        rd=rating_b_1.rd_time_adjusted(),
        pid=rating_b_1.player_id,
    )
    player_b_2 = Glicko2Player(
        rating=rating_b_2.rating,
        rd=rating_b_2.rd_time_adjusted(),
        pid=rating_b_2.player_id,
    )
    ...
    return [
        RatingSnapshot(
            player_id=rating_a_1.player_id,
            timestamp=time.time(),
            rating=player_a_1.rating,
            rd=player_a_1.rd,
            vol=player_a_1.vol,
            game_res_id=game_res_id,
        ),
        RatingSnapshot(
            player_id=rating_a_2.player_id,
            timestamp=time.time(),
            rating=player_a_2.rating,
            rd=player_a_2.rd,
            vol=player_a_2.vol,
            game_res_id=game_res_id,
        ),
        RatingSnapshot(
            player_id=rating_b_1.player_id,
            timestamp=time.time(),
            rating=player_b_1.rating,
            rd=player_b_1.rd,
            vol=player_b_1.vol,
            game_res_id=game_res_id,
        ),
        RatingSnapshot(
            player_id=rating_b_2.player_id,
            timestamp=time.time(),
            rating=player_b_2.rating,
            rd=player_b_2.rd,
            vol=player_b_2.vol,
            game_res_id=game_res_id,
        ),
...
def update_players(
    ...
    if opp_rd_calc == "team_mean":
        ...
        pa1_opp_rd = math.sqrt(
            sum([rd**2 for rd in (player_a_2.rd, player_b_1.rd, player_b_2.rd)])
        )
        pa2_opp_rd = math.sqrt(
            sum([rd**2 for rd in (player_a_1.rd, player_b_1.rd, player_b_2.rd)])
        )

        pb1_opp_rd = math.sqrt(
            sum([rd**2 for rd in (player_b_2.rd, player_a_1.rd, player_a_2.rd)])
        )
        pb2_opp_rd = math.sqrt(
            sum([rd**2 for rd in (player_b_1.rd, player_a_1.rd, player_a_2.rd)])
        )
    else:
        raise ValueError(
            f"Unknown opp_rd_calc mode {repr(opp_rd_calc)}. Allowed are 'team_mean' and '3v1'"
        )

    player_a_1.update_player(
        [team_b_elo],
        [pa1_opp_rd],
        [result_a],
        match_importance=match_importance * team_a_match_importance_factor,
    )
    player_a_2.update_player(
        [team_b_elo],
        [pa2_opp_rd],
        [result_a],
        match_importance=match_importance * team_a_match_importance_factor,
...
def _calc_actual_result(team_points_a, team_points_b, maxv=0.5, fac=6.5, offset=4.5):
    ...
    courtesy = maxv * sigmoid(fac * point_ratio - offset)
