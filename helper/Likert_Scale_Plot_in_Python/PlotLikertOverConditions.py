...
def PlotLikertOverConditions(tb, nPoint, customLikertRange=None, tb2=None, customLikertRange2=None,
    ...
    with sns.axes_style("white"):
        ...
        for cond in df['condition'].unique():
            ...
            middles_all.append(
                temp2[LikertRange[:len(LikertRange) // 2]].sum(axis=1) + temp2[len(LikertRange) // 2 + 1] * .5)
        ...
        if type(tb2) == pd.core.frame.DataFrame:
            ...
            middles_all_generals = temp2[LikertRange[:len(LikertRange) // 2]].sum(axis=1) + temp2[
                len(LikertRange) // 2 + 1] * .5
        ...
        for cond, df_c in enumerate(df_conds):
            ...
            df_c.insert(0, '', (df_c[LikertRange[:len(LikertRange) // 2]].sum(axis=1) + df_c[len(LikertRange) // 2 + 1] * .5 - longest).abs())
            ...
            patch_handles.append(df_c.plot.barh(ax=ax, stacked=True, color=likert_colors, legend=False,
                                                width=barwidth,
                                                edgecolor='white'))  # ,alpha=1.0-(float(cond)/len(df_conds))*0.7
                ...
                for i, p in enumerate(patch_handles[j].get_children()):
                    ...
                    if type(p) == (matplotlib.patches.Rectangle):
                        ...
                        if p.get_height() == barwidth and not (p in patches_already_moved):
                            ...
                            if p.get_width() > 1 and p.get_facecolor()[0:3] != likert_colors[
                                0]:  # p.get_facecolor()!=(1.0, 1.0, 1.0, 1.0):
                                #                             if cond % 2 == 0:
                                #                                 p.set_hatch('\ '*cond)
                                #                             else:
                                #                                 p.set_hatch('/ '*cond)
                                ...
                                patch_handles[j].text(
                                    ...
                                    "{0:.0f}%".format(p.get_width() / df_c.iloc[j, 1:].sum() * 100),
        ...
        CustomLikertLabels_orderd_by_y_axis = [
            customLikertRange[key] if (customLikertRange != None and customLikertRange.get(key))
            else ['very low', 'very high']
            for key in (ax.get_yticks() + 1)]

        if type(tb2) == pd.core.frame.DataFrame:
            CustomLikertLabels_orderd_by_y_axis = [customLikertRange2[key] if (
                        customLikertRange2 != None and customLikertRange2.get(key))
                                                   else ['very low', 'very high']
