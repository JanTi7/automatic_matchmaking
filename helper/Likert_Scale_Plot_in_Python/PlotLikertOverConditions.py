...
def PlotLikertOverConditions(
    tb,
    nPoint,
    customLikertRange=None,
    tb2=None,
    customLikertRange2=None,
    show=False,
    save_to=None,
    full_q_texts=None,
    eng=True,
    ...
    with sns.axes_style("white"):
        ...
        likert_colors = [(0.0, 0.0, 0.0, 0.0)] + likert_colors
        ...
        for cond in df["condition"].unique():
            ...
            middles_all.append(
                temp2[LikertRange[: len(LikertRange) // 2]].sum(axis=1)
                + temp2[len(LikertRange) // 2 + 1] * 0.5
            )
        ...
        if type(tb2) == pd.core.frame.DataFrame:
            ...
            for gQ, g_col in temp.iteritems():
                ...
                for i in range(1, nPoint + 1):
                    temp2.loc[gQ, i] = g(
                        g_col.astype(np.int32, inplace=True).value_counts(), i
                    )
        ...
        if len(np.array(middles_all_generals)) == 0:
            ...
            longest = max(
                max(map(max, np.array(middles_all))),
                max(np.array(middles_all_generals)),
            )
        ...
        for cond, df_c in enumerate(df_conds):
            ...
            for i in range(len(df_c)):
                df_c.iloc[i, :] /= df_c.iloc[i, :].sum() / 20
            df_c.insert(
                0,
                "",
                (
                    df_c[LikertRange[: len(LikertRange) // 2]].sum(axis=1)
                    + df_c[len(LikertRange) // 2 + 1] * 0.5
                    - longest
                ).abs(),
            )

            ## addition: make relative

            complete_longest = int(
                longest + (df_c[:].sum(axis=1).max() - longest)
            )  # in our case is 16
            # complete_longest *= 2
            ...
            )  # ,alpha=1.0-(float(cond)/len(df_conds))*0.7
            ...
            for j in range(len(patch_handles)):
                for i, p in enumerate(patch_handles[j].get_children()):
                    if type(p) == (matplotlib.patches.Rectangle):
                        if (
                            p.get_height() == barwidth
                            and p not in patches_already_moved
                        ):
                            ...
                            if (
                                p.get_width() > 1
                                and p.get_facecolor()[0:3] != likert_colors[0]
                            ):  # p.get_facecolor()!=(1.0, 1.0, 1.0, 1.0):
                                #                             if cond % 2 == 0:
                                #                                 p.set_hatch('\ '*cond)
                                #                             else:
                                #                                 p.set_hatch('/ '*cond)
                                ...
                                patch_handles[j].text(
                                    ...
                                    "{0:.0f}%".format(
                                        p.get_width() / df_c.iloc[j, 1:].sum() * 100
                                    ),
                                    # "{0:.0f}%".format(p.get_width() / sum([pp.get_width()
                                    #                                        for pp in patch_handles[j].get_children()
                                    #                                        if type(pp) == (matplotlib.patches.Rectangle)]) * 100),
                                    ha="center",
                                    fontdict=font1,
                                )  # .set_zorder(-1)

            patches_already_moved = (
                patches_already_moved + patch_handles[j].get_children()
            )

        yticks = list(ax.get_yticks())
        #     print customLikertRange
        CustomLikertLabels_orderd_by_y_axis = [
            customLikertRange[key]
            if (customLikertRange != None and customLikertRange.get(key))
        ...
        if type(tb2) == pd.core.frame.DataFrame:
            CustomLikertLabels_orderd_by_y_axis = [
                customLikertRange2[key]
                if (customLikertRange2 != None and customLikertRange2.get(key))
                else ["very low", "very high"]
                for key in range(1, len(df_conds_generals) + 1)
            ][::-1] + CustomLikertLabels_orderd_by_y_axis
                        ...
                        if (
                            ...
                            if (
                                ...
                                patch_handles[j].text(
                                    ...
                                    "{0:.0f}%".format(
                                        p.get_width()
                                        / (len(tb) / len(tb["condition"].unique()))
                                        * 100
                                    ),
                                    ha="center",
                                    fontdict=font1,
        ...
        z = ax.axvline(longest, linestyle="-", color="black", alpha=0.5, linewidth=1)
        z.set_zorder(-1)
        #     print longest

        X_MARG = 0.5
        plt.xlim(-X_MARG, complete_longest + X_MARG)
        # plt.xlim(50, complete_longest + X_MARG)
        ...
        general_Qs = (
            []
            if len(df_conds_generals) == 0
            else df_conds_generals.index.values.tolist()
        )  # +['']
        ylabels = general_Qs + full_q_texts
