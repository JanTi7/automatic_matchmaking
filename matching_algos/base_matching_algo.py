...
class MatchupCostCalculator:
    def __init__(
        self,
        rating_list,
        played_together_game,
        played_together_team,
        player_ids: list[str],
    ...
    def from_taskinput(cls, task_input: TaskInput):
        return cls(
            task_input.rating_list,
            task_input.weight_game,
            task_input.weight_team,
    ...
    def convert_pids_to_abs(self, games: list[tuple[str]]) -> list[tuple[int]]:
        return [tuple(self.player_ids.index(pid) for pid in game) for game in games]

    def abs_to_pids_dict(self, d: dict[int, Any]) -> dict[str, Any]:
        return {self.player_ids[i]: v for i, v in d.items()}
    ...
    def _reduce_played_together(self, val):
        return self._reduce_if_below_threshold(
            val, self.reduction_threshold_played_together
        )

    def elo_gap(self, games: list[tuple[int]], exponent=1):
        return self.factor_elo_gap * sum(
            [
                self._reduce_elo_gap(self.ratings[min(g)] - self.ratings[max(g)])
                ** exponent
                for g in games
            ]
        )

    def _elo_gap_wrt_single_person(
        self, person, games: list[tuple[int]], exponent=1, raw=False
    ):
        relevant_games = [g for g in games if person in g]
        results = list()
        for game in relevant_games:
            results.append(
                max([abs(self.ratings[p] - self.ratings[person]) for p in game])
            )
        ...
        return self.factor_elo_gap * sum(
            [self._reduce_elo_gap(r) ** exponent for r in results]
        )

    def elo_gap_duo(self, idx1, idx2, exponent=1):
        return (
            self.factor_elo_gap
            * self._reduce_elo_gap(abs(self.ratings[idx1] - self.ratings[idx2]))
        ) ** exponent
    ...
    def _played_together_wrt_single_person(
        self, person, games: list[tuple[int]], exponent=1, raw=False
    ):
        relevant_games = [g for g in games if person in g]
        results = list()
        for game in relevant_games:
            ...
            assert (
                len(opponents) == 2
            ), f"{person=} {teammate_idx=} {teammate=} {game=} {len(opponents)=} {opponents=}"
        ...
        return self.factor_played_together * sum(
            [self._reduce_played_together(r) ** exponent for r in results]
        )

    def played_together_team_duo(self, idx1, idx2, exponent=1):
        return (
            self.factor_played_together
            * self._reduce_played_together(self.played_together_team[idx1][idx2])
        ) ** exponent
    ...
    def cost_per_person(self, games: list[tuple[int]]) -> dict:
        ...
        for person in itertools.chain(*[t for t in games]):
            ...
            costs[person] = MatchupCost(
                ...
                played_together=(
                    pt := self._played_together_wrt_single_person(person, games, 1)
                ),
                total=eg + td + pt,
            )
    ...
    def cost_per_person_raw(self, games: list[tuple[int]]) -> dict:
        ...
        for person in itertools.chain(*[t for t in games]):
            ...
            costs[person] = MatchupCost(
                elo_gap=(
                    eg := self._elo_gap_wrt_single_person(person, game, 1, raw=True)
                ),
                team_diff=(td := self.team_diff(game, 1, raw=True)),
                played_together=(
                    pt := self._played_together_wrt_single_person(
                        person, games, 1, raw=True
                    )
                ),
                total=eg + td + pt,
            )
    ...
    def teammate_cost_matrix(
        self, exponent=1
    ) -> np.array:  # tuple[np.array, list[str]]:
        cost_matrix = np.zeros((len(self.ratings), len(self.ratings)))

        wmean = weighted_mean_with_w(self.higher_rating_weight)
        all_team_ratings = [
            (wmean([self.ratings[i], self.ratings[j]]), frozenset((i, j)))
        ...
        for i in range(len(self.ratings)):
            for j in range(i + 1, len(self.ratings)):
                ...
                other_teams_ratings = [
                    rating
                    for rating, team in filter(
                        lambda t: len(t[1].union(frozenset([i, j]))) == 4,
                        all_team_ratings,
                    )
                ]

                diff_ratings = [
                    abs(team_rating - other_team_rating)
                    for other_team_rating in other_teams_ratings
                ]
                team_diff_estimate = (
                    self.factor_team_diff * self._reduce_team_diff(min(diff_ratings))
                ) ** exponent
    ...
    def print_detailed_cost_analysis(self, taskoutput: TaskOutput):
        ...
        idx2name = {
            taskoutput.input.player_ids.index(pid): name
            for pid, name in id2name.items()
        }
        ...
        for matchup in taskoutput.matchups_as_idx():
            ...
            print(
                f"Max: {idx2name[min(matchup)]} ({ratings[min(matchup)]}) - Min: {idx2name[max(matchup)]} ({ratings[max(matchup)]})"
            )
            print(f"{diff=}, {reduction=}, {with_factor=}, {with_exponent=}")

            assert with_factor == single_cost.elo_gap
            assert (
                with_exponent == single_cost_quad.elo_gap
            ), f"{with_exponent=}, {single_cost_quad.elo_gap=}"
            ...
            assert (
                with_exponent == single_cost_quad.team_diff
            ), f"{with_exponent=}, {single_cost_quad.elo_gap=}"
            ...
            assert (
                with_exponent == single_cost_quad.played_together
            ), f"{with_exponent=}, {single_cost_quad.elo_gap=}"
...
class BaseMatchingAlgo(ABC):
    ...
    def find_matching(
        self, task_input: TaskInput, log_task=False, *args, **kwargs
    ) -> TaskOutput:
        ...
        logging.debug(
            f"{self.name} returned the following matchup: {output.matchups} | {output.matchups_as_idx()}"
        )
