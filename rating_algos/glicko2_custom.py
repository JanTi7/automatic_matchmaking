...
class Glicko2Custom(BaseRatingAlgo):
    ...
    def __init__(self, start_rating, start_rd, start_vol, rubberband, mif, opp_rd_calc, inc_rd_with_time,
                 higher_rating_weight, result_softener, inc_rd_maxv=155, inc_rd_inc_factor=25, inc_rd_max_inc=100):
        super().__init__()
        ...
        self.initial_state = defaultdict(lambda: Glicko2Player(start_rating, start_rd, start_vol))
                ...
                if self.inc_rd_with_time:
                    for pid in game[:4]:  # just the 4 players
                        if pid in last_game_ts:
                            self.players[pid].rd = time_adjust_rd(self.players[pid].rd,
    ...
    def predict(self, x: list[X_data]) -> list[Prediction]:
        return [
            Prediction(None, 0) for game in x]
    ...
    def predict_raw(self, x: list[X_data], higher_rating_weight) -> list[PredictionRaw]:
        return [
            ...
            for game in x
            if (win_team_a := glicko2predict(self.players[game.a1],
                                             self.players[game.a2],
                                             self.players[game.b1],
                                             self.players[game.b2],
