...
from dao import \
    get_all_names, PlayerPool, get_player_from_id, add_new_player, load_player_pool_interactive
...
def make_completer(list_of_player_ids_in_pool, list_of_players_voluntarily_pausing):
    ...
    already_in_pool = {f"{e.first_name} {e.last_name}"
                       for e in [get_player_from_id(p_id) for p_id in list_of_player_ids_in_pool]}
    currently_paused = {f"{e.first_name} {e.last_name}"
                       for e in [get_player_from_id(p_id) for p_id in list_of_players_voluntarily_pausing]}
    return NestedCompleter.from_nested_dict({
        'add': {
            name: None for name in all_names.difference(already_in_pool)
        },
        ...
        'remove': {
            name: None for name in already_in_pool
        },
        ...
        'unpause': {
            name: None for name in currently_paused
        },
...
def manage_the_pool(conf):
    ...
    while True:
        ...
        text = prompt('# ', completer=make_completer(pool.list_of_player_ids, pool.players_voluntarily_pausing),
        ...
        if cmd == "table":
            ...
            if args in names:
                ...
                print(f"Unknown Player {repr(args)}. Use the add_new command to add new players.")
                continue
            ...
            initial_rating = int(prompt("Initial Rating: ", default=str(conf.init_rating)))

            parts = args.split(" ")
            if len(parts) == 2:
                new_id = add_new_player(parts[0], parts[1], init_rating=initial_rating, init_rd=conf.init_rd)

            else:
                name_with_comma = prompt("Please add a comma between first and last name: ",
                                         default=args)  # could add validator checks for exactly one comma
                first, last = name_with_comma.split(",", 1)
                new_id = add_new_player(first, last, init_rating=initial_rating, init_rd=conf.init_rd)
            ...
            gameblock, task_output = pool.start_next_round(conf.pause_mode, num_sets="interactive",
